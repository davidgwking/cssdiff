...
var mismatchTolerance = Number(util.defaultsTo(casper.cli.get('mismatchTolerance'), 0.05));
  ...
  casper.viewport(1024, 768).then(function () {
    ...
    casper.eachThen(files, function (testFile) {
      ...
      casper.thenOpen(testFile.path, function () {
        ...
        verboseInfo('Saved screenshot to ' + testFile.screenshotPath);
      });
    });
  });
  ...
  casper.run(function end() {
    ...
    if (rebase) {
      ...
      files.forEach(function (file) {
        util.copyFile(file.screenshotPath, file.baselinePath);
      });
...
function regressionTest(testFile) {
  ...
  verboseInfo('Comparing ' + screenshotPath + '\n' +
              '\tto mapped baseline file ' + baselinePath + '\n' +
              '\tsaving diff to ' + diffPath + '\n' +
              '\tsaving failures to ' + failurePath);

  var exists = util.isFile(baselinePath);
  if (!exists) {
    addSkip();
  ...
  casper.thenOpen(resembleContainerPath, function () {
    ...
    casper.then(function () {
      casper.fill('form#image-diff-form', {one: baselinePath, two: screenshotPath});
    });
...
function getBaselinePath(input, inputsRoot) {
  return path.join(baselinesDirectory, getRelativePath(input, inputsRoot) + '.png');
}